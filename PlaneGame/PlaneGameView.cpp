...
void CPlaneGameView::AI()
{
	...
	if(nCreator<=0)
	{
		nCreator = rand()%5+5;//敌机数目
		m_ObjList[enEnemy].AddTail(new CEnemy);//产生新的敌机
	}
	nCreator--;
	...
	if(GetKey(VK_SPACE)==1)//按下了空格键
	{
		if(m_pMe!=NULL && m_pMe->Fired())
		{
			...
			m_ObjList[enBomb].AddTail(new CBomb(pt.x+10,pt.y+10));//一发子弹
	...
	for(POSITION ePos=m_ObjList[enEnemy].GetHeadPosition();ePos!=NULL;)
	{
		  ...
		  //if(by && ePt.x >= PlanePt.x && ePt.x<PlanePt.x+CMyPlane::PLANE_WIDTH)//此语句控制敌机开火条件
		  //{
